(function (Date, undefined) {
    ...
    Ansi_Up.prototype.setup_palette = function () {
        ...
        (function () {
            ...
            var format = function (_r, _g, _b) { return levels[_r] + ', ' + levels[_g] + ', ' + levels[_b] };
            var r, g, b;
            for (r = 0; r < 6; ++r) {
                for (g = 0; g < 6; ++g) {
                    for (b = 0; b < 6; ++b) {
        ...
        (function () {
            var level = 8;
            var format = function (x) {return x + ',' + x + ',' + x};
            var i;
    ...
    Ansi_Up.prototype.linkify = function (txt, fileUrlCallback) {
        txt = txt.replace(/(https?:\/\/[^\s\"\'\\]+)/gm, function(str) {
            return buildLink(str, str);
        });
        
        if (fileUrlCallback) {
            txt = txt.replace(/(file:\/\/\/[^\s\"\'\\]+)/gm, function (str) {
                var path = str.substr(8);
                var fileName = path.split('\\').pop().split('/').pop();
                var fileUrl = fileUrlCallback(path);
                return buildLink(fileUrl, fileName);
            });
        }
    ...
    ansi_up = {
        ...
        linkify: function (txt, fileUrl) {
            var a2h = new Ansi_Up();
            return a2h.linkify(txt, fileUrl);
        },
